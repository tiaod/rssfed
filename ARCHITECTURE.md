...
2. **离线优先的订阅管理与阅读** — 服务端 CouchDB + 客户端 PouchDB 双向同步，支持离线缓存、已读/收藏标记，联网后自动合并
...
关键设计：服务端用 CouchDB 原生 `_replicate` API 做 Global → User 的过滤复制，客户端 PouchDB 做 User → 浏览器的实时同步。是两个独立层级。这一分层保证了**离线优先**——浏览器 IndexedDB 是始终可用的数据源，联网时自动与 CouchDB 同步，用户在任何网络状态下都能顺畅阅读和操作。
...
1. **RSS 资讯聚合**：用户发现并订阅感兴趣的 feed → rss-parser 解析 → 写入 CouchDB Global（去重）→ BullMQ 定时调度周期性拉取更新
2. **用户同步**：查订阅列表 → 触发 sync API → CouchDB \_replicate Global → User DB（过滤仅用户订阅的内容）→ PouchDB 同步到浏览器（离线可用）
3. **离线阅读**：浏览器 PouchDB 本地缓存 → 离线浏览、搜索 → 标记已读/收藏 → 联网后自动合并推回 Per-User CouchDB
4. **ActivityPub 分发**：BotKit Bot 定期扫描用户订阅的新条目（或手动触发）→ 根据用户配置筛选符合条件的条目 → 构建 Create Activity → 推送到 Bot 的 follower inbox（Fediverse）
